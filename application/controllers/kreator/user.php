...
class User extends CI_Controller {
    ...
    public function profil() {
        ...
        if ($this->input->post()) { // Jika terdapat data yang disubmit masuk ke kondisi ini
          ...
          $this->form_validation->set_rules($config);
            ...
            if ($this->input->post('password') != NULL) {
              $data ['kreator_password'] = md5($this->input->post('password'));
            }
            $this->kreator_model->update($this->session->userdata('kreator_id'), $data);

            // Update data admin yang sedang login
            $user_data['kreator_nama'] = $this->input->post('nama');
            $user_data['kreator_username'] = $this->input->post('username');
            $user_data['kreator_email'] = $this->input->post('email');
            $user_data['kreator_telefon'] = $this->input->post('telefon');
            $user_data['kreator_alamat'] = $this->input->post('alamat');
    ...
    public function update($id) {
        ...
        if ($validation == FALSE) {
            return redirect('kreator/user/profil/'.$id);
        }
